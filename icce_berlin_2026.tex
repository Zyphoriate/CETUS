...
\title{Coupled Electrochemical and Component-Level Modeling for Real-Time Smartphone Battery Life Prediction}
...
\textbf{Power coefficients.} The per-component coefficients $\{\alpha_k\}$ and baseline $P_{\mathrm{base}}$ in \eqref{eq:pload} are estimated by linear regression on ADB \texttt{batterystats} traces.
...
The proposed model tracks the true TTE consistently across all cases, including on Honor and Xiaomi where device-specific coefficients were not fitted. No prediction deviates by more than $2\times$ from the ground truth.
...
On the fitting device (Huawei) and on Xiaomi, our model wins all six scenarios with \SI{4.0}{\percent}--\SI{43.1}{\percent} reduction over the best baseline. Gains are largest on video profiles, where the ECM provides a structural advantage over statistical extrapolation.

On Honor, the other held-out device, the model wins on Video (\SI{11.3}{\percent}) but trails E-MANAFA on Gaming (\SI{1.7}{\percent} margin) and Mixed (\SI{13.3}{\percent}), indicating that per-device re-calibration of $\{\alpha_k\}$ would improve cross-device transfer.
...
Fig.~\ref{fig:ablation_tte} reports the ablation results across three devices and nine scenarios. Each bar compares Ours (the full version) against two stripped variants: one without temperature correction (w/o Temp) and one without ECM dynamics (w/o ECM).
